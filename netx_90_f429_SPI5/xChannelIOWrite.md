...
`xChannelIOWrite` 함수는 `xChannelIORead`와 쌍을 이루는 cifX 툴킷 API 함수로, 특정 통신 채널의 I/O 출력 영역(Output Area)에 데이터를 쓰는 데 사용됩니다. 이 함수 역시 DPM(Dual-Port Memory) 직접 접근 방식과 DMA(Direct Memory Access) 전송 방식을 모두 지원합니다.
...
1.  **파라미터 유효성 검사:**
    ...
    *   `ulAreaNumber`: 요청된 I/O 영역 번호가 유효한 출력 영역 범위 내에 있는지 확인합니다. 범위를 벗어나면 `CIFX_INVALID_PARAMETER`를 반환합니다.
    *   `ulOffset + ulDataLen`: 쓰려는 데이터의 총 크기(오프셋 + 길이)가 해당 I/O 영역의 전체 크기를 초과하는지 확인합니다. 초과하면 `CIFX_INVALID_ACCESS_SIZE` 오류를 반환합니다.
...
3.  **데이터 쓰기 (DPM 모드):**
    *   **핸드셰이크(Handshake) 모드:**
        ...
        *   `DEV_WaitForBitState()` 함수를 사용하여 DPM의 핸드셰이크 플래그가 호스트가 데이터를 쓸 수 있는 상태가 될 때까지 `ulTimeout` 시간 동안 대기합니다. 이는 netX 펌웨어가 이전에 호스트가 쓴 데이터를 처리했음을 보장합니다.
        *   타임아웃이 발생하면 `CIFX_DEV_EXCHANGE_FAILED` 오류를 반환합니다.
        *   핸드셰이크가 성공하면 `HWIF_WRITEN()` 매크로를 사용하여 사용자 버퍼(`pvData`)의 데이터를 DPM의 지정된 오프셋에 복사합니다.
        *   `DEV_ToggleBit()` 함수를 호출하여 핸드셰이크 비트를 토글(toggle)함으로써, 호스트가 새로운 데이터를 DPM에 썼음을 netX 펌웨어에 알립니다.
    *   **핸드셰이크 없는(Non-Handshake) 모드:**
        *   별도의 대기 과정 없이 `HWIF_WRITEN()`을 사용하여 즉시 DPM에 데이터를 씁니다.

4.  **데이터 쓰기 (DMA 모드):**
    *   DMA 모드에서는 I/O 영역 0만 지원됩니다. 다른 영역을 요청하면 `CIFX_DEV_DMA_IO_AREA_NOT_SUPPORTED` 오류를 반환합니다.
    *   DPM 모드의 핸드셰이크 방식과 유사하게 `DEV_WaitForBitState()`로 DMA 버퍼가 비워지기를 기다립니다.
    *   `OS_Memcpy()`를 사용하여 사용자 버퍼의 데이터를 호스트 메모리에 있는 DMA 버퍼로 복사합니다.
    *   `DEV_ToggleBit()`로 핸드셰이크 비트를 토글하여 DMA 쓰기 완료를 알립니다.
...
int32_t APIENTRY xChannelIOWrite(CIFXHANDLE hChannel, uint32_t ulAreaNumber, uint32_t ulOffset, uint32_t ulDataLen, void* pvData, uint32_t ulTimeout)
{
...
#endif
  {
    ...
    {
      HWIF_WRITEN(ptChannel->pvDeviceInstance, &ptIOArea->pbDPMAreaStart[ulOffset], pvData, ulDataLen);
...
`xChannelIOWrite`는 netX 디바이스로 출력 데이터를 전송하는 표준화된 방법을 제공하는 저수준(low-level) 함수입니다. `xChannelIORead`와 마찬가지로 핸드셰이크 메커니즘을 통해 데이터 교환의 동기화를 맞추고, DPM과 DMA 두 가지 하드웨어 통신 방식을 모두 지원하여 유연성을 제공합니다. 이 함수는 제어 명령을 액추에이터로 보내는 등의 작업에 필수적입니다.
