...
static uint8_t ValidateDipSwitchData(void);
/* USER CODE END PFP */
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
{
  ...
  {
    // 에러 처리
    HAL_UART_Transmit(&huart5, (uint8_t*)"UART Error!\r\n", 13, HAL_MAX_DELAY);
  }
}
...
static uint8_t ValidateDipSwitchData(void)
{
    ...
    else
    {
        ...
        memcpy(secondDipData, currentDipData, 5);
        
        HAL_UART_Transmit(&huart5, (uint8_t *)"Second DIP data: ", 17, HAL_MAX_DELAY);
        HAL_UART_Transmit(&huart5, secondDipData, 4, HAL_MAX_DELAY);
        HAL_UART_Transmit(&huart5, (uint8_t *)"\r\n", 2, HAL_MAX_DELAY);
        
        // Compare first and second receptions
        if (strcmp((const char*)firstDipData, (const char*)secondDipData) == 0)
        {
            // Match found - store validated data
            memcpy((void*)g_DipSwitchData, firstDipData, 5);
            g_bDipDataValid = 1;
        ...
        else
        {
            ...
            firstDataReceived = 0;
            memset(firstDipData, 0, 5);
            memset(secondDipData, 0, 5);
