...
########################################################################################
# $Id: hilscher_netx.py 1144 2025-05-06 13:23:29Z AMesser $:
#
...
HILSCHER_VERSION = '1.14.0.5'

from waflib import Configure, Errors, Options, Utils, Task, Node, Logs, Context, ConfigSet
...
def load_hilscher_or_default(ctx, load, tool_list, tooldir = None, **kw):
    ''' Load tool from hilscher tools if available, otherwise load default tools '''
    if tooldir is None:
        for tool in Utils.to_list(tool_list):
            tooldir = [ hilscher_waf_dir + os.sep + 'tools', hilscher_waf_dir ]
...
''' ------------------------------------------
    ...
    ------------------------------------------ '''
def options(conf):
    conf.add_option('--conditions', choices="debug debugrel release".split(), default=None,
                help=u'Select a build condition (debug, debugrel or release[default] )')

    # By default, build artifacts will be placed in hilscher specific
    # file structure within dist folder
    conf.parser.set_defaults(
        prefix='',
        destdir=os.environ.get('DESTDIR', 'dist')
    )

    conf.add_option('--dump-environment', default='',
                    help='Log selected environment variables to file (e.g. "DEFINES,CFLAGS')

    conf.load('hilscher_devices hilscher_waf_console')
...
''' configuration stuff    '''
def configure(conf):
    conf.msg('Waf tool version', Context.WAFVERSION)
    ...
    conf.load('hilscher_devices hilscher_waf_console objcopy')
...
# a bunch of helpers for error reporting
def format_file_message(ctx, msg, node):
    try:
        base_node = ctx.launch_node()
    except AttributeError:
        base_node = ctx.srcnode

    return 'In %r: %s' % (node.path_from(base_node), msg)

@conf
def file_warning(ctx, msg, node):
    Logs.pprint('YELLOW', 'Warning: ' + format_file_message(ctx,msg, node))

@conf
def file_error(ctx, msg, node):
    raise ctx.errors.WafError(format_file_message(ctx,msg, node))

@conf
def wscript_warning(ctx, msg, path = None):
    ctx.file_warning(msg, ctx.get_wscript(path))
...
@taskgen_method
def wscript_warning(self, msg):
    prefix = 'Target %r: ' % (self.name)
    self.bld.wscript_warning(prefix + msg, path = self.path)
...
@taskgen_method
def hil_ensure_posted(self):
    if not getattr(self, 'posted', False):
        self.post()

match_target      = re.compile(r'(?P<target_triplet>[^/]+)/(?P<cc_version>[0-9a-z.]+)/(?P<target>.+)$').match

@conf
def hil_get_tgen_by_name(self, name):
    ''' Implement hilscher specific tasg generator name resolution
...
class LazyTaskGeneratorNotFoundError(Errors.WafError):
    ''' Implements lazy generation of error msg to avoid overhead of running difflib if not needed'''
    def __init__(self, name, task_gen):
        super(LazyTaskGeneratorNotFoundError, self).__init__()
        self.task_gen = task_gen
        self.name = name

    @property
    def msg(self):
        cur_script = self.task_gen.cur_script or self.task_gen.path.find_node('wscript')
        known_str = ''
        ...
        try:
            ...
            similar_known_keys = sorted(self.task_gen.task_gen_hilscher_cache_names.keys(),\
                                        key=lambda x : SequenceMatcher(None,x,self.name).ratio(),\
                                        reverse=True)[:10]

            if similar_known_keys:
                known_str = '. Did you mean ' + ','.join('%r' % x for x in similar_known_keys) + '?'

        self.__dict__['msg'] = msg = 'Undefined task generator name %r in %r%s' % (self.name, cur_script.nice_path(), known_str)
        return msg

    @msg.setter
    def msg(self, value):
        ''' Make WafError constructor happy'''
        pass

@conf
def get_tgen_by_name(self, name):
    ''' Get task generator by name and print a user friendly error message

        Overrides the original implementation of this waf function in order
        to produce a more use friendly output.
    '''
    try:
        return self.hil_get_tgen_by_name(name)
    except KeyError:
        raise LazyTaskGeneratorNotFoundError(name, self)
...
def get_build_stamp(self):
    ''' Retrieve the timestamp to use for this build '''
    try:
        return self.bld.buildstamp
    except AttributeError:
        self.bld.buildstamp = datetime.datetime.today().replace(microsecond=0)
        return self.bld.buildstamp
...
def add_dist_nodes(self, nodes):
    ''' Consider node(s) for distribution

        This is a helper function which will register the provided node(s) for
        later use by distribution tools. Since this function is used from many places,
        it is made generally available here.
    '''
...
@feature('bootimage', 'nxf', 'nxo', 'hboot', 'netx90_app_image')
@after_method('build_bootimage','build_nxf', 'build_nxo', 'build_hboot', 'build_netx90_app_image', 'build_nai', 'build_nxi_mxf')
def perform_legacy_install(self):
    ''' Performs the legacy install method provided in old hilscher waf script versions

        This method is intended for backwards compatibility to legacy build command install_path
        usage in many of existing projects.
    '''

    install_path = getattr(self, 'install_path', None)
    if install_path:
      self.bld.install_files(install_path, self.dist_nodes)
...
def sdkcomponent(bld, name, **kw):
    ...
    bld(SDK=True,
        ...
        features=features + ['use', 'includes', 'sdk'],
        **kw)

@feature('sdk')
def is_sdk(self):
    ''' Check function to test for sdk and to prevent warnings '''
    return ('sdk' in self.features) or getattr(self,'SDK', False)
...
def stlib(bld, target, toolchain=None, platform=None, **kw):
    ...
    process_hilscher_source(bld, kw)
...
'''
@conf
def svn_get_info(conf, path_or_url):
    """
    ...
    if isinstance(path_or_url, conf.node_class):
    ...
    try:
        out = conf.cmd_and_log(
                  conf.cmd_to_list(conf.env.SVN) + ['info' , path_or_url],
...
def process_hilscher_source(bld, kw):
...
class dumpenv(Task.Task):
    def run(self):
        ...
        with open(outputs[0].abspath(),'wt') as out:
            header = ''
...
def add_tsk_to_dumpenv_task(self, tsk):
    ...
    if opt.dump_environment:
        if isinstance(opt.dump_environment, str):
            dump_vars = opt.dump_environment.split(',')
        else:
            dump_vars = opt.dump_environment
        ...
        if ((not getattr(self,"hidden_from_list", False)) or 'cprogram' in self.features):
            ...
            if not hasattr(self,"dumpenv_task"):
                ...
                dumpenv_task.vars = list(dump_vars)
...
class ListContext(BuildContext):
  ...
  def execute(self):
    ...
    # omit sdks
    tgens = (tg for tg in tgens if not tg.is_sdk())
    ...
    for tg in tgens:
        ...
        if toolchain is None:
            ...
            while work_list and (toolchain is None):
                try:
                    x = self.hil_get_tgen_by_name(work_list.pop(0))
                except KeyError:
                    pass
                else:
...
class netx_bootimage(Task.Task):
    ...
    def run(self):
      ...
      boot_node = self.outputs[0]
      bin_node = boot_node.change_ext('objdump.bin')

      # Use Objcopy to generate binary file
      cmd = self.env['OBJCOPY'] + ['-O', 'binary', inputfile, bin_node.abspath()]
      result = self.generator.bld.cmd_and_log(cmd, output=STDOUT, quiet=STDOUT)

      # Get the application data.
      with open(bin_node.abspath(), 'rb') as f:
        strBinFile = f.read()
      ...
      # This is slow but will work on python2 & python3
      for offset in range(0, len(strBinFile), 4):
        data, = struct.unpack_from("<L", strBinFile, offset)

        ulApplicationChecksum += data
      ...
      # Write the bootimage.
      with open(boot_node.abspath(), 'wb') as f:
        f.write(struct.pack('<16L', *aBootBlock))
        f.write(strBinFile)
...
def build_bootimage(self):
    ...
    for x in self.to_list(self.use):
        try:
            tg = self.bld.hil_get_tgen_by_name(x)
            # is already posted?
            if not getattr(tg, 'posted', None):
                tg.post()
            break
...
@feature('programm', 'asm', 'c', 'cxx', 'hboot', 'bootimage', 'fake_lib')
...
def check_tgen_availability(self):
    ...
    for x in self.to_list(getattr(self, "use", [])):
        try:
            t = self.bld.hil_get_tgen_by_name(x)
        except KeyError:
            tgen_disabled = "use-component \'%s\' for target \'%s\' not found." % (x, self.name)
        else:
            tgens.append(t)

    if not tgen_disabled:
        for t in tgens:
            if not getattr(t, "posted", None):
                t.post()

            try:
                tgen_disabled = tgen_disabled or t.tgen_disabled
            except AttributeError:
                # external sdk components are currently not associated any taskgen_function, thus posting them doesnt do anything
                # and this check becomes a false positive
                if not t.is_sdk():
                    msg = "task_gen {t.name} is missing tgen_disabled attribute when processing tgen {self.name}".format(**locals())
...
def process_use(self):
    ...
    for x in list(use_prec.keys()):
        try:
            y = self.bld.hil_get_tgen_by_name(x)
        except KeyError:
            pass
        else:
    ...
    if getattr(self, 'link_task', None):
      ...
      for u in getattr(self,"use", []):
        ...
        y = self.bld.hil_get_tgen_by_name(u)
