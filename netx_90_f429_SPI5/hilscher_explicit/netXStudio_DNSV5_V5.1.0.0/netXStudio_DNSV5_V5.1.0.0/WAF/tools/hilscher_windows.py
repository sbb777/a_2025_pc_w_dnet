...
########################################################################################
# $Id: hilscher_windows.py 1047 2024-11-19 08:45:52Z AMesser $:
#
...
def exec_command(self, cmd, **kw):
    ...
    if cmd_length > max_commandline_len:
        ...
        # Handle cwd like original waf would do (WAF-291)
        cwd = kw.get('cwd', '.')
        ...
        (fd, path) = tempfile.mkstemp('.lnk', dir=os.path.abspath(cwd), text=False)
        try:
            # we must handle the encoding here since python2's mkstemp function does not support 'encoding' argument
            os.write(fd, (params + '\n').encode('utf-8'))
        finally:
            os.close(fd)
