...
########################################################################################
# $Id: hilscher_toolchain.py 1036 2024-10-31 07:12:36Z AMesser $:
#
...
@conf
def alias_toolchain(conf, name, alias):
    for x in Utils.to_list(alias):
        conf.all_envs['toolchain_%s' % x] = conf.all_envs['toolchain_%s' % name].derive()
...
def setup_gnu_binutils(conf, path_list = None):
    ...
    for tool in 'ar readelf objdump objcopy strip nm'.split():
        ...
        exe = conf.find_program(executable_name, var=tool.upper(), path_list=path_list)

        # Ensure on waf1.6 this is a list!
        env[tool.upper()] = conf.cmd_to_list(exe)

    env.ARFLAGS = 'rcs'

@conf
def gcc_arm_optimize_flags(conf):
    f = conf.env.append_value

    # The ORDER of the flags MATTERS! Optimization adjustsments must
    # be made after -O setting.

    # WAF-288: Mitigation for GCC bug
    # https://gcc.gnu.org/bugzilla/show_bug.cgi?id=38644
    if (4,2,4) < conf.cc_version() < (4,6,0):
        extra_flags = ['-fno-schedule-insns2']
    else:
        extra_flags = []

    if conf.env.CONDITIONS == 'debug':
        f('CFLAGS__optimize',   ['-O0'] + extra_flags)
        f('CXXFLAGS__optimize', ['-O0'] + extra_flags)
    else:
        f('CFLAGS__optimize',   ['-Os'] + extra_flags)
        f('CXXFLAGS__optimize', ['-Os'] + extra_flags)
