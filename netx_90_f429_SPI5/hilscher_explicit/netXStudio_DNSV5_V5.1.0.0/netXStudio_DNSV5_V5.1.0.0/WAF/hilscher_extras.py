...
def options(conf):
    conf.add_option('--generate-report', action='append', dest='generate_report',
...
def generatewscript(bld, target, use, *k, **kw):
    ...
    try:
        tgen = bld.hil_get_tgen_by_name(name)
    except KeyError:
...
def build_wscript(self):
    ...
    for x, includes in self.use.copy().items():
        try:
            tgen = self.bld.hil_get_tgen_by_name(x)
        except KeyError:
            del new_use[x]
        else:
    ...
    def run(self):
      ...
      for tgen in input:
        if not getattr(tgen, "posted", None):
            self.generator.bld.fatal("Source tgen not yet posted")
...
def process_sourcezip(self):
    ...
    while use:
        try:
            tgen = self.bld.hil_get_tgen_by_name(use.pop(0))
        except KeyError:
            continue
        ...
        if "fake_lib" in tgen.features:
            ...
            if (name + "_arm_objs") in tgen_use:
                source_arm.extend(self.to_list(
                    self.bld.hil_get_tgen_by_name(name + "_arm_objs").source))

            if (name + "_thumb_objs") in tgen_use:
                source_thumb.extend(self.to_list(
                    self.bld.hil_get_tgen_by_name(name + "_thumb_objs").source))
