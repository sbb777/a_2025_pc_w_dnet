...
########################################################################################
# $Id: hilscher_doc.py 1139 2025-04-07 12:12:37Z RHornung $:
#
# Description:
...
from hilscher_compat import always_run
...
def configure(conf):
    conf.load('hilscher_compat hilscher_plantuml')
...
class doxygen(Task.Task):
    ...
    def run(self):
        ...
        # enforce hhc enabled
        if env['HTMLHELPC']:
            doxyfile_content += '\nHHC_LOCATION = "%s"\n' % env.get_flat('HTMLHELPC')

        # set plantuml if found
        if env['PLANTUML']:
            doxyfile_content += '\nPLANTUML_JAR_PATH = "%s"\n' % env.get_flat('PLANTUML')
