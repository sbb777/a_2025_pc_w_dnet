...
def make_array32(data=''):
    u''' Make an array of 32 bit unsigned integers '''
    return list(struct.unpack('<%dL' % (len(data) / 4), data))

def array_to_bytes(data_list):
    return struct.pack('<%dL' % len(data_list), *data_list)

def update_hash(m, data_list):
    m.update(array_to_bytes(data_list))
...
def generate_commonheader_md5_checksum(filedata, header_offset = 0):
    ...
    offset_common_header         = 16
    offset_common_header_md5          = offset_common_header + 5
    offset_common_header_commoncrc32  = offset_common_header + 12
    offset_common_header_crc32        = offset_common_header + 15
    ...
    # set common header common crc32 to zero
    filedata[header_offset_words + offset_common_header_commoncrc32] = 0
    ...
    # Calculate file MD5
    m = hashlib.md5()
    update_hash(m, filedata[header_offset_words:])
    file_md5_string = m.digest()

    file_md5 = make_array32(file_md5_string)
...
def generate_commonheader_headercrc32_checksum(filedata, header_offset = 0):
    ...
    header_crc32 = zlib.crc32(array_to_bytes(filedata[header_offset_words:header_offset_words+32])) & 0xffffffff
    ...
    if success:
      ...
      if taglist_elfname:
        ...
        while _offset < len(taglist_data):
          ...
          _eolfound = False
          if (_tagtype == 0) and (_taglength == 0):
            _eolfound = True
