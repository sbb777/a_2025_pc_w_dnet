...
########################################################################################
# $Id: hilscher_libsused.py 995 2024-10-01 12:50:30Z AMesser $:
#
# Description:
#
...
def build_libsused_common(self, output, generators, referenced_node, install_name):
    ...
    for tg in generators:
        ...
        if not hide_from_usedlibs:
            ...
            elif tg.is_sdk() and getattr(self,'include_SDK', False):
...
@feature('libsused')
@after_method('apply_link','build_nxo','build_nxf', 'build_nxi_mxf')
...
class LibsusedTask(Task.Task):
    ...
    def extractVersionSection(self, tg, node):
        ...
        if len(err) > 0:
            ...
            version_string = codecs.decode(hexversion,'hex').partition(b'\x00')[0].decode().strip()
    ...
    def run(self):
        ...
        for tg, x in self.input_tgens:
            ...
            description = getattr(tg, 'description', None) or '<No description>'
