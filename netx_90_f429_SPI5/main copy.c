...
//  HAL_UART_Receive_IT(&huart5, &rxByte, 1);
  ...
  static uint8_t accumulated_line[200] = {0};
  static uint16_t accumulated_len = 0;
  ...
  while (!g_bDipDataValid)
  {
      ...
      if (len > 0)
      {
          ...
          // Debug: Show line length
          char lenStr[16];
          sprintf(lenStr, "Len: %d\r\n", len);
          HAL_UART_Transmit(&huart5, (uint8_t *)lenStr, strlen(lenStr), HAL_MAX_DELAY);

          // Accumulate partial lines until we get a complete DIP pattern
          if (accumulated_len + len < sizeof(accumulated_line) - 1) {
              // Copy new data to accumulated buffer
              for(int i = 0; i < len; i++) {
                  accumulated_line[accumulated_len + i] = line[i];
              }
              accumulated_len += len;
              accumulated_line[accumulated_len] = '\0';
              
              HAL_UART_Transmit(&huart5, (uint8_t *)"Accumulated: ", 13, HAL_MAX_DELAY);
              HAL_UART_Transmit(&huart5, accumulated_line, accumulated_len, HAL_MAX_DELAY);
              HAL_UART_Transmit(&huart5, (uint8_t *)"\r\n", 2, HAL_MAX_DELAY);
              
              // Try to validate with accumulated data
              if (ValidateDipSwitchData(accumulated_line) == 1)
              {
                  HAL_UART_Transmit(&huart5, (uint8_t *)"D data validated!\r\n", 19, HAL_MAX_DELAY);
                  break; // Exit validation loop
              }
              
              // If accumulated line contains D pattern but validation failed,
              // or if it gets too long, reset
              if (strstr((const char*)accumulated_line, "D:[") != NULL || accumulated_len > 150) {
                  accumulated_len = 0;
                  accumulated_line[0] = '\0';
                  HAL_UART_Transmit(&huart5, (uint8_t *)"Buffer reset\r\n", 14, HAL_MAX_DELAY);
              }
          } else {
              // Buffer overflow prevention - reset
              accumulated_len = 0;
              accumulated_line[0] = '\0';
              HAL_UART_Transmit(&huart5, (uint8_t *)"Buffer overflow - reset\r\n", 25, HAL_MAX_DELAY);
          }
      } else {
