...
int main(void)
{
  ...
  // 첫 메시지 전송
//  HAL_UART_Transmit(&huart5, txMessage, strlen((char*)txMessage), HAL_MAX_DELAY);
...
//  HAL_UART_Receive_IT(&huart5, &rxByte, 1);
  ...
  // DIP switch validation loop - receive and compare twice
  ...
  while (!g_bDipDataValid)
  {
      ...
      if (len > 0)
      {
          ...
          // Debug: Show line length and hex dump
          char lenStr[16];
          sprintf(lenStr, "Len: %d\r\n", len);
          HAL_UART_Transmit(&huart5, (uint8_t *)lenStr, strlen(lenStr), HAL_MAX_DELAY);

          // Validate DIP switch data (requires 2 consecutive matches)
          ...
          HAL_UART_Transmit(&huart5, (uint8_t *)"No data received\r\n", 18, HAL_MAX_DELAY);
      }

      HAL_Delay(100); // Short delay for validation loop
  }

////    if (rxReady)
//    {
//      // 받은 문자 하나를 에코백
//      HAL_UART_Transmit(&huart5, &rxByte, 1, HAL_MAX_DELAY);
//
//      // 다음 문자 받기 재시작
//      HAL_UART_Receive_IT(&huart5, &rxByte, 1);
//
...
static uint8_t ExtractDipSwitchData(const uint8_t *line, uint8_t *dipData)
{
    // Look for pattern "DIP:[XXXX]" where XXXX is 4 binary digits
    const char *dipStart = strstr((const char*)line, "DIP:[");
    
    if (dipStart == NULL) {
        HAL_UART_Transmit(&huart5, (uint8_t *)"No DIP pattern found\r\n", 22, HAL_MAX_DELAY);
        return 0;
    }
    
    dipStart += 5; // Move to start of binary digits
    
    // Validate 4 binary digits
    for (int i = 0; i < 4; i++)
    {
        if (dipStart[i] != '0' && dipStart[i] != '1') {
            HAL_UART_Transmit(&huart5, (uint8_t *)"Invalid DIP digit at pos ", 26, HAL_MAX_DELAY);
            char posStr[2] = {'0' + i, '\0'};
    ...
    HAL_UART_Transmit(&huart5, (uint8_t *)"DIP extracted: ", 15, HAL_MAX_DELAY);
...
/*!
 * \brief Validate DIP switch data by comparing two consecutive receptions
 * \param[in] currentLine - Current received UART line
 * \return 1 if validation successful, 0 if need more data
 */
static uint8_t ValidateDipSwitchData(const uint8_t *currentLine)
{
    static uint8_t firstDipData[5] = {0};
    static uint8_t secondDipData[5] = {0};
    static uint8_t firstDataReceived = 0;
    ...
    // Extract DIP switch data from current line
    if (ExtractDipSwitchData(currentLine, currentDipData) == 0) {
        HAL_UART_Transmit(&huart5, (uint8_t *)"DIP extraction failed\r\n", 23, HAL_MAX_DELAY);
        return 0; // No valid DIP data in current line
    }
    
    if (firstDataReceived == 0)
    {
        // Store first reception
        for(int i = 0; i < 5; i++) firstDipData[i] = currentDipData[i];
        firstDataReceived = 1;
        
        HAL_UART_Transmit(&huart5, (uint8_t *)"First DIP data: ", 16, HAL_MAX_DELAY);
        HAL_UART_Transmit(&huart5, firstDipData, 4, HAL_MAX_DELAY);
        HAL_UART_Transmit(&huart5, (uint8_t *)"\r\n", 2, HAL_MAX_DELAY);
        
        return 0; // Need second reception
    }
    else
    {
        // Store second reception
        for(int i = 0; i < 5; i++) secondDipData[i] = currentDipData[i];
        
        HAL_UART_Transmit(&huart5, (uint8_t *)"Second DIP data: ", 17, HAL_MAX_DELAY);
        HAL_UART_Transmit(&huart5, secondDipData, 4, HAL_MAX_DELAY);
        HAL_UART_Transmit(&huart5, (uint8_t *)"\r\n", 2, HAL_MAX_DELAY);
        
        // Compare first and second receptions manually
        uint8_t match = 1;
        for(int i = 0; i < 4; i++) {
            if(firstDipData[i] != secondDipData[i]) {
                match = 0;
                break;
            }
        }
        ...
        if (match == 1)
        {
            // Match found - store validated data
            for(int i = 0; i < 5; i++) g_DipSwitchData[i] = firstDipData[i];
            g_bDipDataValid = 1;
            
            HAL_UART_Transmit(&huart5, (uint8_t *)"DIP validation SUCCESS: ", 24, HAL_MAX_DELAY);
        ...
        else
        {
            ...
            HAL_UART_Transmit(&huart5, (uint8_t *)"DIP validation MISMATCH - retrying...\r\n", 40, HAL_MAX_DELAY);
